...
def query_table(access_token, sql):
    headers = {
        'Authorization': f'Zoho-oauthtoken {access_token}',
        'ZANALYTICS-ORGID': ORG_ID
    }
    url = f'https://analyticsapi.zoho.com/restapi/v2/workspaces/{WORKSPACE_ID}/data'
    params = {
        'CONFIG': json.dumps({
            'responseFormat': 'json',
            'sqlQuery': sql
    ...
    print(f'Query response: {r.text[:400]}')
    try:
        result = r.json()
...
def main():
    ...
    print('Consultando Segmentación acumulada Total...')
    seg = query_table(token, 'SELECT * FROM "Segmentación acumulada Total"')
    ...
    print('Consultando Transiciones de segmentos Total...')
    trans = query_table(token, 'SELECT * FROM "Transiciones de segmentos Total"')
